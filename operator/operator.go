...
// CleanupFiles removes all target files from the repository. It is intended to be
// used after file restoration or command execution to ensure no sensitive files
...
func (o *Operator) Run(command string, environ []string) error {
	...
	// Resolve shell
	sh, flag := resolveShell()
	cmd := exec.Command(sh, flag, command) // #nosec G204
	cmd.Dir = dir
	cmd.Stdin = os.Stdin
...
// resolveShell returns the shell and flag to use for executing commands.
